...
RSpec.describe 'API/V1/Questions', type: :request do
  ...
  let(:answers_one) { create_list(:answer, 4, questions.first) }
  let(:answers_two) { create_list(:answer, 4, questions.last) }
  ...
  describe 'POST /quizzes/{:quiz_id}/questions' do
    ...
    context 'invalid params' do
      let!(:invalid_params) { attributes_for(:question).except(:content) }
      before { post "api/v1/quizzes/#{quiz.id}/questions", params: valid_params, headers: headers }
  ...
  describe 'GET /quizzes/{:quiz_id}/questions/{:id}' do
    ...
    it 'returns question' do
      expect(json['question']).not_to be_empty
    end

    it 'returns question answers' do
      expect(json['question']['answers']).not_to be_empty
    end
  end

  describe 'PATCH /quizzes/{:quiz_id}/questions/{:id}' do
    let!(:updated_question) { attributes_for(:question, content: 'New content!') }
    before { patch "/api/v1/quizzes/{:quiz_id}/questions/{:id}", headers: headers }
  ...
  describe 'DELETE /quizzes/{:quiz_id}/questions/{:id}' do
    before { delete "/api/v1/quizzes/#{quiz.id}/questions/#{questions.last.id}" }
